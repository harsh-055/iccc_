...
class LocalAuthUserResponseDto {
  ...
  @ApiProperty({
    description:
      'Information about the tenant the user is currently logged in with',
    example: {
...
export class LocalauthController {
  ...
  @ApiBody({
    type: LoginDto,
    description:
      'User login credentials with optional tenant and system role selection',
    examples: {
      standard: {
  ...
  refreshToken(@Req() req: Request, @Body() Body: any) {
    return this.localAuthService.refreshToken(req, {
  ...
  @ApiResponse({
    status: 200,
    description: 'MFA QR code image returned successfully.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns QR code and setup instructions when user needs to set up MFA during login. This is a public endpoint that requires valid credentials.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - MFA not enabled for user.',
  })
  @ApiBody({
  ...
  @ApiOperation({
    ...
    description:
      'Verifies that user has successfully set up MFA by providing their first authentication token. This completes the MFA setup process without performing a full login.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid MFA token or setup already complete.',
  })
  @ApiBody({
  ...
  async verifyMfaSetup(
    @Body() data: { email: string; password: string; mfaToken: string },
  ) {
  ...
  @ApiOperation({
    ...
    description:
      'Completes the login process after MFA token verification. Use this when MFA token is required.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - Invalid credentials or MFA token.',
  })
  @ApiBody({
