...
export class LensWhatsAppService implements IWhatsAppService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.lensVerificationWhatsappUrl = this.configService.get(
      'LENS_VERIFICATION_WHATSAPP_URL',
    );
  }
  ...
  async sendMessage(
    ...
    if (!response.ok) {
      this.logger.log(
        `Failed to send message to ${to} with message ${message}`,
      );
      return false;
    }
    this.logger.log(
      `Successfully sent message to ${to} with message ${message}`,
    );
    return true;
  }
  ...
  async sendVerificationCode(to: string) {
    ...
    if (!this.lensVerificationWhatsappUrl) {
      this.logger.error(
        'LENS_VERIFICATION_WHATSAPPA_URL is not defined in environment variables',
      );
      return false;
    }
    if (!this.lensCloudApiKey) {
      this.logger.error(
        'LENS_CLOUD_API_KEY is not defined in environment variables',
      );
      return false;
    }
    ...
    if (!response.ok) {
      ...
      this.logger.error(
        `Failed to send verification whatsapp message to ${to}`,
      );
      return false;
    }
    return true;
  ...
  async verifyVerificationCode(to: string, code: string) {
    ...
    if (!this.verifyWhatsAppUrl) {
      this.logger.error(
        'LENS_VERIFICATION_MAIL_URL is not defined in environment variables',
      );
      return false;
    }
    if (!this.lensCloudApiKey) {
