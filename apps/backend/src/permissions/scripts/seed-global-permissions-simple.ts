...
async function createGlobalPermission(databaseService: DatabaseService, permission: {
  name: string;
  resource: string;
  action: string;
  description: string;
  ...
  try {
    ...
    console.error(`Error creating global permission ${permission.name}: ${error.message}`);
    throw error;
  }
}
